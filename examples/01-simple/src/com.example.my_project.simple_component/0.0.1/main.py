...
from the AWS Greengrass V2 component recipe as shown in:
...
class AwsGreengrassV2Component():
    ...
    def service_loop(self):
        ...
        while True:
            try:
                ...
                # Example: Publish a simple periodic update
                self.simple_update_message()
    ...
    def simple_update_message(self):
        ...
        try:
            ...
            # Create the UPDATE message format
            update_message = self.pubsub_messages.get_pubsub_update(command, status, params)
            
            # Publish the message to this components IPC Data Topic
            # Once deployed, this message will be available to other components on this Greengrass core 
            self.publish_message('ipc', self.ipc_data_topic, update_message)

            # Publish the message to this components MQTT Data Topic
            # Once deployed, this message will be available in the AWS IoT Core
            self.publish_message('mqtt', self.mqtt_data_topic, update_message)

        except Exception as err:
                log.error('EXCEPTION: Exception in simple_update_message - ERROR MESSAGE: {}'.format(err))

if __name__ == "__main__":

    try:
        # Accepts the Greengrass V2 config from deployment recipe into sys.argv[1] as shown in:
        # https://github.com/aws-samples/aws-greengrass-application-framework.git
